...
def main():
    ...
    with st.sidebar: 
        ...
        st.title("Welcome!!!")
        choice = st.radio("Navigation", ["Check the crop type","Know about NPK","Dont know Rainfall in your area?","Want to know selling price of your crop?"])
        st.info("This project application helps you to predict crop type suitable for your soil.")
    
    
    if choice == "Check the crop type":
        st.write(":red[**THe NPK values should in kg/ha,to calculate that multiply each term by 30**]")
        st.caption("So if ratio 2:1:1, Type N=60,P=30,K=30")
        N = st.slider('The N value of NPK values of fertilizer you are using', 0.0, 140.0)
        P = st.slider('The P value of NPK values of fertilizer you are using', 0.0, 130.0)
        K = st.slider('The K value of NPK values of fertilizer you are using', 0.0, 92.5)
        ...
        if st.button('Predict the crop type'):
            with st.spinner("Predicting crop type..."):
                diagnosis = crop_prediction([N, P, K, temperature, humidity, ph, rainfall])
